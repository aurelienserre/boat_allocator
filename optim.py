...
time_prefs = {
    "comp": pd.read_csv(
        "data_processed/comp/time_prefs.csv", header=[0, 1], index_col=0
    ),
    "rec_master": None,
}
...
# for each group, dict with key = level of people, values = list of boat skills
# they can row
boat_class_per_skill = {
    "comp": {1: [0, 1], 2: [2]},
    "rec_master": {1: [1], 2: [2], 3: [2]},
}
...
for b in boat_set:
    ...
    for t in time_set:
        people_for_boat_time[(b, t)] = {
            p for p in people_for_boat if not np.isnan(prefs.loc[p, t])
        }
    ...
    for p in people_set:
        for t in times_avail_for_person[p]:
            boat = {
                b
                for b in boats_for_person[p]
                if model.getVal(variables[(p, b, t)]) == 1
            }
            assert len(boat) <= 1
            if boat:
